<template>
  <select
    v-model="sortOrder"
    class="border-0 focused:border-0 active:border-0 cursor-pointer p-0 pr-8 mb-2 lg:ml-0 sm:ml-auto block"
  >
    <option value="CHEAPEST_FIRST">Сначала дешевые</option>
    <option value="EXPENSIVE_FIRST">Сначала дорогие</option>
  </select>
  <div
    v-if="this.$apollo.queries.products.loading"
    class="products flex flex-row flex-wrap justify-between"
  >
    <div
      class="product-card bg-slate-50 animate-pulse rounded-xl w-64 h-96 box-border p-8 mb-5 flex flex-col border border-gray-200"
      v-for="item in 9"
      :key="item"
    ></div>
  </div>
  ...
  >
    ...
    <ProductCard
      ...
      :key="product.node.id"
      :productData="product"
      :categorySlug="product.node.category.slug"
    ></ProductCard>
      ...
      if (isNext && this.cursor != null) {
        this.$apollo
          .query({
            query: gql`
                    query {
                        products(sortOrder: ${this.sortOrder}, first: 12, searchQuery: "${this.searchQuery}", after: "${endCursors}") {
                            edges {
                                node {
                                    id
                                    name
