<template>
    <select v-model="sortOrder" class="border-0 focused:border-0 active:border-0 cursor-pointer p-0 pr-8 mb-2  lg:ml-0 sm:ml-auto block">
    ...
    <div v-else class="products flex flex-row flex-wrap justify-between" @scroll="onScroll">
        ...
        <ProductCard v-for="product in products.edges" :key="product.node.id" :productData="product" :categorySlug="product.node.category.slug"></ProductCard>
             
        <template v-if="productsMore" >
            <ProductCard v-for="product in productsMore" :key="product.node.id" :productData="product" :categorySlug="product.node.category.slug"></ProductCard>
        </template>
    ...
    apollo: {
        products: {
            query: gql`query products($searchQuery: String!, $sortOrder: ProductSortOrder!) {
                products(sortOrder: $sortOrder, first: 21, searchQuery: $searchQuery) {
                    edges {
                        node {
                            id
                            name
