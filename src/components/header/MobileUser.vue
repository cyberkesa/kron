<template>
    ...
    <div v-if="authInfo.viewer == 'AnonymousViewer'" class="py-10 flex flex-col">
        <p class="text-[15px] mb-6 ">Войдите или зарегистрируйтесь, чтобы совершать заказы</p>
        <router-link class="bg-[#CC5654] text-white rounded-full text=[15px] py-4 w-[100%] text-base font-semibold block text-center hover:opacity-90" to="/login">Войти или Зарегистрировться</router-link>
    </div>
    ...
    <p class="text-lg font-semibold text-[15px]"> {{ authInfo.userEmail }} </p>
        </div>
