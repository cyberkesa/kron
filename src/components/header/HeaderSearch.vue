  ...
  <div class="search-box relative lg:mr-10 sm:mr-0">
    <div class="relative w-full">
      ...
      <input
        ...
        class="bg-slate-100 placeholder:text-[#8185AE] rounded-[10px] h-9 pl-12 lg:w-80 sm:w-full border-0 focus:border-slate-200 focus:bg-white focus:border"
        :class="hasFindClass"
      />
    </div>
    <div
      class="absolute flex flex-col top-[100%] bg-white lg:w-80 sm:w-full rounded-b-[10px] box-border py-2 px-4 border border-slate-200"
      v-if="searchFind"
      ...
      tabindex="0"
    >
      <p v-if="searchResult.length == 0" class="text-[14px]">
        Ничего не найдено
      </p>
      <router-link
        @click="clearResult2()"
        class="font-semibold block mt-2 hover:text-slate-500 cursor-pointer text-[14px]"
        v-for="[key, value] in searchCategories"
        :key="key"
        :to="'/category/' + key"
      >
        {{ value }}
      ...
      <router-link
        @click="clearResult2()"
        class="block mt-2 hover:text-slate-500 cursor-pointer text-[14px]"
        v-for="item in searchResult"
        :key="item"
        :to="
          '/category/' + item.node.category.sluf + '/product/' + item.node.slug
        "
      >
        {{ item.node.name }}
      </router-link>
