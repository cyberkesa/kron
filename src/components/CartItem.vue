<template>
 <div class="cart-item flex flex-row items-center justify-between py-5 pl-3 pr-10 border-b border-r"
     v-if="quantityItem">
     <div class="w-[80px] mr-4 flex lg:justify-center sm:justify-start">
         <img :src="(cartItemData.node.product.mainImage) ? cartItemData.node.product.mainImage.url : '/no-image.jpg'"
             alt="" class="w-auto h-auto max-w-[80px] lg:max-h-[80px] sm:max-h-auto">
     </div>
     <div class="flex lg:flex-row sm:flex-col justify-between items-center w-full">
         <div class="flex justify-between">
             <p class="mr-auto text-[15px] font-normal"> {{ cartItemData.node.product.name }} </p>
             <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
                 @click="deleteFromCart()" class="cursor-pointer hover:opacity-90 ml-11 lg:hidden sm:flex">
         ...
         <div class="flex flex-row justify-between items-center lg:w-auto sm:w-full lg:m-0 sm:mt-4">
             <p class="mr-11 text-[15px] font-semibold whitespace-nowrap lg:ml-4 sm:ml-0">
                 {{ cartItemData.node.product.price * quantityItem }} ₽ </p>
             <AddToCart :productId="cartItemData.node.product.id" :cartId="cartItemData.node.id"
                 @changeQuantity="updateCart($event)"
                 :stockAvailabilityStatus="cartItemData.node.product.stockAvailabilityStatus" :quantityMultiplicity="cartItemData.node.product.quantityMultiplicity" />

             <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
                 @click="deleteFromCart()" class="cursor-pointer hover:opacity-90 ml-11 lg:flex sm:hidden">
                 <path d="M20.0002 1V3H4.00024V1H20.0002Z" fill="#C8CDDF" />
                 <path
                     d="M10.7831 10.485C10.6822 10.0214 10.2623 9.68183 9.77118 9.69899C9.28003 9.71614 8.88491 10.0842 8.81652 10.5537L8.81327 10.576L8.80591 10.7109L9.05177 17.7514L9.06852 17.8855L9.07332 17.9075C9.17429 18.3711 9.59412 18.7107 10.0853 18.6935C10.5764 18.6764 10.9715 18.3083 11.0399 17.8388L11.0432 17.8165L11.0505 17.6816L10.8047 10.6411L10.7879 10.507L10.7831 10.485Z"
                     fill="#C8CDDF" />
