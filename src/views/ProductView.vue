<template>
  <div>
    ...
    >
      <div
        class="product-images rounded-xl p-8 border border-gray-200 lg:w-[582px] sm:w-full lg:m-0 sm:mb-5"
        v-if="productBySlug.mainImage"
      >
        <Swiper
          :navigation="true"
          :pagination="true"
        ...
        >
          <SwiperSlide
            v-for="image in productBySlug.images"
            :key="image.url"
            class="flex flex-col"
            ><img class="lg:h-[490px] sm:h-auto w-auto" :src="image.url"
          /></SwiperSlide>
        </Swiper>
      </div>
      <div
        v-else
        class="product-images rounded-xl border border-gray-200 w-[582px]"
      >
        <img src="/no-image.jpg" class="w-[100%] h-[100%] rounded-xl" />
      </div>
      <div
        class="product-info p-8 rounded-xl border border-gray-200 lg:w-[582px] sm:w-full"
      >
        ...
        <div class="flex justify-between border-b border-gray-200 py-8">
          ...
          <AddToCart
            ...
            :stockAvailabilityStatus="productBySlug.stockAvailabilityStatus"
            :quantityMultiplicity="productBySlug.quantityMultiplicity"
          />
        </div>
