  ...
  <div v-else class="category-view">
    <template v-if="products.edges.length > 0">
      ...
      <p class="text-slate-400 mb-8">
        По вашему запросу
        <b class="text-gray-900">"{{ this.$route.params.searchQuery }}"</b>
        найдены товары:
      </p>
    ...
    <template v-else>
      ...
      <div class="text-clip mt-20">
        ...
        <router-link
          to="/"
          class="bg-[#CC5654] text-white mx-auto rounded-full py-4 w-[160px] text-base font-semibold block text-center hover:opacity-90"
...
export default {
  ...
  apollo: {
    products: {
      query: gql`
        query products($searchQuery: String!, $sortOrder: ProductSortOrder!) {
          products(
            sortOrder: $sortOrder
            first: 20
  ...
  mounted() {
    document.title =
      ...
      '" - Поиск товаров - компания ООО "КРОН"';
  },
  watch: {
    sortOrder() {
      (this.productsMore = []),
    ...
    $route() {
      (this.productsMore = []), (this.sortOrder = "CHEAPEST_FIRST");
    },
  },
  computed: {
    endCursor() {
