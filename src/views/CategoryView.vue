<template>
  <div class="category-view">
    <BreadcrumbsLinks
      v-if="categoryBySlug.id"
      :categoryId="categoryBySlug.id"
    />
    <h1 class="lg:mb-10 sm:mb-6 lg:text-[32px] sm:text-2xl">
    ...
    >
      <div class="sm:w-full lg:w-1/4">
        <router-link
          to="/catalog/"
          class="text-xl flex items-center mb-2 justify-start"
        >
          <svg
            width="16"
            height="16"
          ...
          >
            <path
              d="M4.39134 7.99998L10.196 2.19531L11.1387 3.13798L6.27667 7.99998L11.1387 12.862L10.196 13.8046L4.39134 7.99998Z"
              fill="black"
            />
          </svg>
        ...
        <router-link
          v-if="categoryBySlug.parent"
          :to="categoryBySlug.parent.slug"
          class="text-xl flex items-center mb-2"
        >
          <svg
            width="16"
            height="16"
        ...
        <div v-if="categoryBySlug.children" class="categoires">
          <div
            class="category"
            v-for="category in categoryBySlug.children"
            :key="category.id"
          >
            <router-link
            ...
            >
              ...
              >
                <path
                  d="M4.86133 3.13798L9.72333 7.99998L4.86133 12.862L5.80399 13.8046L11.6087 7.99998L5.80399 2.19531L4.86133 3.13798Z"
                  fill="black"
                />
              </svg>
            </router-link>
  ...
  methods: {
    getTitle() {
      this.$apollo
        ...
        .then((data) => {
          document.title =
            data.data.categoryBySlug.title + ' - компания ООО "КРОН"';
        });
    },
  },
  computed: {
    breadcrumb() {
      ...
      if (this.categoryBySlug.parent) {
        breadcrumbs.push({
          url: this.categoryBySlug.parent.slug,
          title: this.categoryBySlug.parent.title,
        });
      }
      breadcrumbs.push({
        url: this.categoryBySlug.slug,
        title: this.categoryBySlug.title,
      });
      return breadcrumbs;
    },
  },
  ...
  watch: {
    categoryBySlug() {
      this.categoryBySlug
        ...
        : (document.title =
            this.categoryBySlug.title + ' - компания ООО "КРОН"');
